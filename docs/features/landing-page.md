...
- **Newsletter**: an email-capture signup, UI only and off by default. Wire the `<form>` to your email provider.
- **Footer**: links, legal, and social.

## 5. SEO is already handled

Every route renders the `Seo` component (`landing/src/lib/components/Seo.svelte`), so titles, descriptions, self-referencing canonicals, and Open Graph / Twitter cards are set for you. There are no per-page URLs to hardcode. See **[SEO](seo.md)** to customize it.
